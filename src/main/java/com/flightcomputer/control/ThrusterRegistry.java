...
public final class ThrusterRegistry {
    ...
    public void refresh(Level level, BlockPos controllerPos, Map<VectorDirection, BlockPos> stabiliserLinks, Map<VectorDirection, BlockPos> autopilotLinks, long gameTime, Object subLevel) {
        ...
        lastRefreshTick = gameTime; lastSubLevel = subLevel;
    ...
    private void refreshBank(Level level, BlockPos controllerPos, FlightMode mode, Map<VectorDirection, BlockPos> assignments, Object subLevel) {
        Map<VectorDirection, List<ThrusterLink>> bank = links.get(mode);
        for (List<ThrusterLink> value : bank.values()) value.clear();
        if (assignments != null && !assignments.isEmpty()) for (Map.Entry<VectorDirection, BlockPos> entry : assignments.entrySet()) addExplicit(level, controllerPos, mode, entry.getKey(), entry.getValue(), bank, subLevel);
        if (subLevel != null && scanSablePlot(controllerPos, mode, bank, subLevel)) return;
        scanOrdinaryLevel(level, controllerPos, mode, bank);
    }

    private boolean scanSablePlot(BlockPos controllerPos, FlightMode mode, Map<VectorDirection, List<ThrusterLink>> bank, Object subLevel) {
        try {
            ...
            BlockGetter getter = accessor instanceof BlockGetter g ? g : null;
            if (getter == null) {
                ...
                Object fallback = ctor.newInstance(plot);
                if (!(fallback instanceof BlockGetter g)) return false;
                getter = g;
            }
    ...
    private void scanOrdinaryLevel(Level level, BlockPos controllerPos, FlightMode mode, Map<VectorDirection, List<ThrusterLink>> bank) {
        BlockPos min = controllerPos.offset(-DISCOVERY_RADIUS, -DISCOVERY_RADIUS, -DISCOVERY_RADIUS), max = controllerPos.offset(DISCOVERY_RADIUS, DISCOVERY_RADIUS, DISCOVERY_RADIUS);
    ...
    private static void discoverCompatible(BlockEntity blockEntity, double[] offset, FlightMode mode, Map<VectorDirection, List<ThrusterLink>> bank) {
        for (VectorDirection direction : VectorDirection.values()) {
            ...
            if (!(source instanceof ReflectivePropulsionSource reflective) || reflective.getPhysicalDirection() != direction) continue;
            addIfUnique(bank.get(direction), new ThrusterLink(source, direction, mode));
        }
    }

    private void addExplicit(Level level, BlockPos controllerPos, FlightMode mode, VectorDirection direction, BlockPos storedTarget, Map<VectorDirection, List<ThrusterLink>> bank, Object subLevel) {
        if (storedTarget == null || direction == null) return;
        BlockEntity direct = level.getBlockEntity(storedTarget);
        PropulsionSource directSource = direct == null ? null : ReflectivePropulsionSource.tryCreate(direct, direction, mountOffset(controllerPos, storedTarget));
        if (directSource != null) { addIfUnique(bank.get(direction), new ThrusterLink(directSource, direction, mode)); return; }
        if (subLevel != null) {
            BlockEntity subLevelEntity = getSubLevelBlockEntity(subLevel, storedTarget);
            if (subLevelEntity != null) {
                PropulsionSource source = ReflectivePropulsionSource.tryCreate(subLevelEntity, direction, new double[]{storedTarget.getX(), storedTarget.getY(), storedTarget.getZ()});
                if (source != null) { addIfUnique(bank.get(direction), new ThrusterLink(source, direction, mode)); return; }
            }
        }
    ...
    private static Object resolveSubLevel(Level level, BlockPos controllerPos) {
        try {
            ...
            Class<?> sable = Class.forName("dev.ryanhcode.sable.companion.SableCompanion", false, ThrusterRegistry.class.getClassLoader());
            Object instance = sable.getField("INSTANCE").get(null);
            if (instance == null) return null;
            try { return instance.getClass().getMethod("getContaining", BlockEntity.class).invoke(instance, controller); } catch (NoSuchMethodException ignored) { }
            try { return instance.getClass().getMethod("getContaining", Level.class, Vec3.class).invoke(instance, level, Vec3.atCenterOf(controllerPos)); } catch (NoSuchMethodException ignored) { }
    ...
    private static BlockEntity getBlockEntity(BlockGetter getter, BlockPos pos) {
        try {
            Object value = getter.getClass().getMethod("getBlockEntity", BlockPos.class).invoke(getter, pos);
    ...
    private static Method findNoArg(Class<?> type, String name) { try { Method method = type.getMethod(name); return method.getParameterCount() == 0 ? method : null; } catch (ReflectiveOperationException | RuntimeException ignored) { return null; } }
    private static Object invokeNoArg(Object target, String name) { if (target == null) return null; Method method = findNoArg(target.getClass(), name); if (method == null) return null; try { return method.invoke(target); } catch (ReflectiveOperationException | RuntimeException ignored) { return null; } }
    private static Integer invokeInt(Object target, String name) { Object value = invokeNoArg(target, name); return value instanceof Number n ? n.intValue() : null; }
    private static double[] mountOffset(BlockPos controllerPos, BlockPos target) { return new double[]{target.getX()+0.5D-(controllerPos.getX()+0.5D), target.getY()+0.5D-(controllerPos.getY()+0.5D), target.getZ()+0.5D-(controllerPos.getZ()+0.5D)}; }
    private static void addIfUnique(List<ThrusterLink> list, ThrusterLink link) { if (list == null || link == null || link.source == null) return; String id = link.source.getId(); for (ThrusterLink existing : list) if (existing.source.getId().equals(id)) return; list.add(link); }
    ...
    public List<ThrusterLink> getAllLinks(FlightMode mode) { LinkedHashMap<String, ThrusterLink> unique = new LinkedHashMap<>(); for (VectorDirection direction : VectorDirection.values()) for (ThrusterLink link : links.get(mode).get(direction)) unique.putIfAbsent(link.source.getId(), link); return List.copyOf(unique.values()); }
    public List<ThrusterLink> getLinks(FlightMode mode, ControlAxis axis) { List<ThrusterLink> result = new ArrayList<>(); for (VectorDirection direction : VectorDirection.values()) { if (axis == ControlAxis.VERTICAL && direction != VectorDirection.UP && direction != VectorDirection.DOWN) continue; if (axis == ControlAxis.LONGITUDINAL && direction != VectorDirection.NORTH && direction != VectorDirection.SOUTH) continue; if (axis == ControlAxis.LATERAL && direction != VectorDirection.EAST && direction != VectorDirection.WEST) continue; if (axis.isRotational() || axis.isTranslational()) result.addAll(links.get(mode).get(direction)); } return Collections.unmodifiableList(result); }
    public List<ThrusterLink> getAllLinks() { LinkedHashMap<String, ThrusterLink> unique = new LinkedHashMap<>(); for (FlightMode mode : FlightMode.values()) for (VectorDirection direction : VectorDirection.values()) for (ThrusterLink link : links.get(mode).get(direction)) unique.putIfAbsent(link.source.getId(), link); return List.copyOf(unique.values()); }
    public double getVectorAuthority(FlightMode mode, VectorDirection direction) { double total=0; for (ThrusterLink link : links.get(mode).get(direction)) total+=Math.max(0,link.source.getAvailableThrust()); return total; }
