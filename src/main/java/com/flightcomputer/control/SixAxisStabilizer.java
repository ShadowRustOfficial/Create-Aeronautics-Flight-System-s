...
public final class SixAxisStabilizer {
    ...
    /**
     * Computes acceleration/force commands while adapting controller response to the live
     * vehicle authority. Large Sable vessels naturally have much lower acceleration per unit
     * control input than small craft; using the same PID response on both makes the large craft
     * saturate its actuators and hunt around the setpoint. The authority-aware scale keeps the
     * same control law but slows the response as the available physical acceleration falls.
     */
    public Map<ControlAxis, Double> computeCommands(VehicleState state, StabilizationSetpoint sp, double dt,
        ...
        // AxisPID returns acceleration/angular-acceleration when called with unit scale. Live
        // inertia is then applied once, while authority-aware scaling prevents large vessels
        // from repeatedly demanding torque that their actuator geometry cannot produce.
    ...
    private static double linearAuthority(ThrusterRegistry registry, FlightMode mode, ControlAxis axis, double mass) {
        if (registry == null) return 6.0D;
        return registry.getAxisAuthority(mode, axis) / Math.max(mass, 1.0e-3D);
    }

    private static double angularAuthority(ThrusterRegistry registry, FlightMode mode, VehicleState state, ControlAxis axis) {
    ...
    private static double responseScale(double authority, double reference) {
        if (!Double.isFinite(authority) || authority <= 0.0D) return 0.35D;
        double ratio = Math.max(0.0D, authority) / Math.max(reference, 1.0e-3D);
        // Square-root scheduling avoids making large craft excessively sluggish while still
        // preventing a low-authority vessel from driving the allocator into constant saturation.
        return clamp(Math.sqrt(ratio), 0.35D, 1.25D);
    }
