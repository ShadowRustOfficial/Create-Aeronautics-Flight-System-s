...
/** Final allocator. Uses the live sub-level centre of mass for torque authority. */
public final class ThrustAllocator {
    ...
    public void applyCombined(ThrusterRegistry registry, VehicleState state,
        ...
        for (Map.Entry<String, PropulsionSource> previous : lastActiveSources.entrySet()) if (!unique.containsKey(previous.getKey())) previous.getValue().applyThrust(0.0D);
        ...
        if (target.normSquared() <= 1.0e-12 || unique.isEmpty()) { hardStop(); return; }
        ...
        for (int iteration = 0; iteration < ITERATIONS; iteration++) {
            ...
            for (int i = 0; i < sources.size(); i++) {
                ...
                double[] contribution = contribution(sources.get(i), available, vehicleRotation, state);
                double dot = 0.0D, magnitude = 0.0D;
                for (int k = 0; k < 6; k++) { double residual = target.component(k) - achieved[k]; dot += residual * contribution[k]; magnitude += contribution[k] * contribution[k]; }
                if (magnitude <= 1.0e-9) continue;
                commands[i] = clamp(commands[i] + dot / magnitude, 0.0D, 1.0D);
                achieved = achieved(sources, commands, vehicleRotation, state);
            }
        }
    ...
    public void hardStop() {
        for (PropulsionSource source : lastActiveSources.values()) try { source.applyThrust(0.0D); } catch (RuntimeException ignored) { }
        lastActiveSources.clear(); lastThermalLoad=0.0D; resetLastWrench();
    }
    ...
    private double[] seedVectorBanks(List<ThrusterLink> sources, ControlWrench target, Quaterniond rotation) {
        ...
        for(VectorDirection direction:VectorDirection.values()){
            double authority=0.0D; Vector3d unitForce=null;
            for(ThrusterLink link:sources) if(link.direction==direction){
                double available=Math.max(0.0D,link.source.getAvailableThrust()); if(available<=0.0D)continue; authority+=available;
                if(unitForce==null){unitForce=new Vector3d(direction.x(),direction.y(),direction.z());rotation.transform(unitForce).mul(link.polarity);double length=unitForce.length();if(length>1e-9)unitForce.div(length);}
            }
            ...
            for(int i=0;i<sources.size();i++){ThrusterLink link=sources.get(i);if(link.direction==direction&&link.source.getAvailableThrust()>0.0D)commands[i]=fraction;}
        }
        return commands;
    }
    ...
    private double[] achieved(List<ThrusterLink> links,double[] commands,Quaterniond rotation,VehicleState state){
        ...
        for(int i=0;i<links.size();i++){double[] c=contribution(links.get(i),links.get(i).source.getAvailableThrust(),rotation,state);for(int k=0;k<6;k++)result[k]+=c[k]*commands[i];}
        return result;
    }

    private double[] contribution(ThrusterLink link,double thrust,Quaterniond rotation,VehicleState state){
        VectorDirection d=link.direction;
        Vector3d force=new Vector3d(d.x(),d.y(),d.z()).mul(thrust*link.polarity);
        // Registry offsets are measured from the controller block. Torque must instead be taken
        // around Sable's live centre of mass, otherwise moving the COM silently changes the
        // sign/magnitude of the stabilising moment.
    ...
    private void resetLastWrench(){lastWorldForceX=lastWorldForceY=lastWorldForceZ=0.0D;lastWorldTorqueX=lastWorldTorqueY=lastWorldTorqueZ=0.0D;}
