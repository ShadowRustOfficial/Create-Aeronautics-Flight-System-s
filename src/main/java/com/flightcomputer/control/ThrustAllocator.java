...
public final class ThrustAllocator {
    ...
    private static void addStabiliserSources(ThrusterRegistry registry, Map<ControlAxis, Double> commands, Map<String, ThrusterLink> unique) {
        if (nonZero(commands.get(ControlAxis.VERTICAL))) addDirectionSources(registry, VectorDirection.UP, unique, FlightMode.STABILIZE);
    ...
    private static void addDirectionSources(ThrusterRegistry registry, VectorDirection direction, Map<String, ThrusterLink> unique, FlightMode mode) {
        for (ThrusterLink link : registry.getLinks(mode, direction)) if (link != null && link.source != null) unique.putIfAbsent(link.source.getId(), link);
    }
    private static boolean nonZero(Double value) { return value != null && Double.isFinite(value) && Math.abs(value) > 1.0e-6D; }
    ...
    private double[] contribution(ThrusterLink link,double thrust,VehicleState state,Quaterniond rotation){VectorDirection d=link.direction;Vector3d force=new Vector3d(d.x(),d.y(),d.z()).mul(thrust*link.polarity);Vector3d r=new Vector3d(link.source.getMountOffset()).sub(state.comX,state.comY,state.comZ);rotation.transform(force);rotation.transform(r);double fx=force.x,fy=force.y,fz=force.z;return new double[]{fx,fy,fz,r.y*fz-r.z*fy,r.z*fx-r.x*fz,r.x*fy-r.y*fx};}
    ...
    private static double totalTorqueAuthority(List<ThrusterLink> sources,VehicleState state,Quaterniond rotation){double total=0;for(ThrusterLink link:sources){double thrust=Math.max(0,link.source.getAvailableThrust());if(thrust<=0)continue;Vector3d force=new Vector3d(link.direction.x(),link.direction.y(),link.direction.z()).mul(thrust*link.polarity);Vector3d r=new Vector3d(link.source.getMountOffset()).sub(state.comX,state.comY,state.comZ);rotation.transform(force);rotation.transform(r);total+=r.cross(force,new Vector3d()).length();}return total;}
    private void resetLastWrench(){lastWorldForceX=lastWorldForceY=lastWorldForceZ=0;lastWorldTorqueX=lastWorldTorqueY=lastWorldTorqueZ=0;}
