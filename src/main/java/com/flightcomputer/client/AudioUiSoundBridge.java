...
/** Routes generic/non-controller-action UI audio to the server-authoritative controller block. */
public final class AudioUiSoundBridge {
    ...
    public static boolean isFlightComputerScreen(Screen screen) {
        return screen instanceof NavigationConsoleScreen || screen instanceof ThermalConsoleScreen
                || screen instanceof CoolingConsoleScreen || screen instanceof FlightOperationsScreen;
    }
    ...
    public static void playForButton(Button button) {
        ...
        if (text.equals("EMERGENCY SHUTDOWN") || text.equals("INSERT HELD") || text.equals("REMOVE")) return;
        if (mc.screen instanceof NavigationConsoleScreen && isControllerActionButton(text)) return;
        if (isTabButton(text)) { FlightComputerUiSoundNetwork.request(pos, soundId(Kind.TAB)); return; }
    ...
    private static boolean isControllerActionButton(String text) {
        return text.equals("SYSTEM") || text.startsWith("SYSTEM:") || text.equals("STABILISER") || text.startsWith("STABILISER:")
                || text.equals("MODE") || text.startsWith("MODE:") || text.equals("AUTOPILOT") || text.startsWith("AUTOPILOT:")
                || text.equals("ALTITUDE HOLD") || text.startsWith("ALTITUDE HOLD:") || text.equals("HEADING HOLD") || text.startsWith("HEADING HOLD:")
                || text.equals("POSITION HOLD") || text.startsWith("POSITION HOLD:") || text.equals("VELOCITY HOLD") || text.startsWith("VELOCITY HOLD:")
                || text.equals("NAVIGATION") || text.startsWith("NAVIGATION:") || text.equals("START ROUTE") || text.equals("ABORT ROUTE")
                || text.equals("DISPLAY TEST") || text.equals("F") || text.equals("B") || text.equals("U") || text.equals("D") || text.equals("L") || text.equals("R");
    }

    private static boolean isTabButton(String text) {
        return text.equals("MAP") || text.equals("ROUTE") || text.equals("FLIGHT CONTROL") || text.equals("DIAGNOSTICS")
                || text.equals("THERMAL") || text.equals("COOLING") || text.equals("NAVIGATION") || text.equals("IDENTITY")
                || text.equals("COMBAT") || text.equals("LANDING") || text.equals("DOCKING") || text.equals("SYSTEM");
    }

    private static Kind toggleKind(Screen screen, FlightControllerBlockEntity controller, String text) {
        if (text.startsWith("TERRAIN:") || text.startsWith("FLIGHT MAP:") || text.startsWith("WAYPOINTS:")
                || text.startsWith("STABILISER AMBIENT:") || text.startsWith("MAP CONTACT:"))
        ...
        if (screen instanceof FlightOperationsScreen && text.endsWith(" HOLD") && controller instanceof FlightOperationsHolder holder) {
            String holdName = text.substring(0, text.length() - 5).trim();
            try { return holder.getFlightOperations().hasHold(FlightHold.valueOf(holdName)) ? Kind.TOGGLE_OFF : Kind.TOGGLE_ON; }
    ...
    private static FlightControllerBlockEntity controller(Screen screen) {
        Minecraft mc = Minecraft.getInstance(); if (mc == null || mc.level == null) return null;
        BlockPos pos = controllerPos(screen); if (pos == null) return null;
