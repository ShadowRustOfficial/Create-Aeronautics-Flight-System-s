...
/** Live diagnostics overlay for the Navigation Console, including the full thermal state. */
...
public final class FlightComputerTelemetryClient {
    ...
    @SubscribeEvent public static void render(RenderGuiEvent.Post event){
        ...
        var t=get(fc.getControllerId());if(t==null)return;
        GuiGraphics g=event.getGuiGraphics();
        int width=360,height=168,left=Math.max(8,mc.getWindow().getGuiScaledWidth()-width-12),top=Math.max(8,mc.getWindow().getGuiScaledHeight()-height-12);
