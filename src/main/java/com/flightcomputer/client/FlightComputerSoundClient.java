...
public final class FlightComputerSoundClient {
    ...
    private static final float GHOST_AMBIENT_VOLUME = 0.15F;
    ...
    public static void tick() {
        ...
        } else if (activeTakeoff != null && !activeTakeoff.isStopped()) activeTakeoff.setTelemetry(best);
    ...
    private static void stopAmbient() { if(activeTakeoff!=null)activeTakeoff.stopNow(); if(activeFlightLoop!=null)activeFlightLoop.stopNow(); if(activeGhostAmbient!=null)activeGhostAmbient.stopNow(); activeTakeoff=null;activeFlightLoop=null;activeGhostAmbient=null;activeController=null;activeTicks=0;takeoffStarted=false; }
    ...
    private static abstract class PositionalLoop extends AbstractTickableSoundInstance {
        ...
        private PositionalLoop(SoundEvent sound, FlightComputerNetwork.TelemetryPayload telemetry, float maxVolume, boolean looping) {
            super(sound, SoundSource.BLOCKS, RandomSource.create()); this.telemetry=telemetry; this.maxVolume=maxVolume; this.looping=looping; this.delay=0; this.volume=maxVolume; this.pitch=1.0F; this.attenuation=SoundInstance.Attenuation.LINEAR; this.relative=false; updatePosition();
        }
        ...
        protected final void setTelemetry(FlightComputerNetwork.TelemetryPayload telemetry){if(telemetry!=null)this.telemetry=telemetry;}
        protected final void setFade(float fade){this.fade=Math.max(0.0F,Math.min(1.0F,fade));}
        private void updatePosition(){if(telemetry==null){stop();return;}this.x=telemetry.x();this.y=telemetry.y();this.z=telemetry.z();}
        protected final void stopNow(){stop();}
    }
    private static final class AmbientLoop extends PositionalLoop { private AmbientLoop(SoundEvent sound, FlightComputerNetwork.TelemetryPayload telemetry, float volume){super(sound,telemetry,volume,true);} }
    private static final class OneShotLoop extends PositionalLoop { private OneShotLoop(SoundEvent sound, FlightComputerNetwork.TelemetryPayload telemetry, float volume){super(sound,telemetry,volume,false);} }
}
